    ...
    def run(self, contract_text):
       response = client.chat.completions.create(
            ...
            messages=[
                ...
                "Analyze the contract for potentially problematic legal terms, clauses, or language "
                "that could create legal issues or disputes. Focus on liability, rights, obligations, and ambiguous language."},
...
class ComplianceValidator:
    ...
    def run(self, contract_text):
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a compliance expert specializing in regulatory requirements across industries. "
                "Analyze the contract for potential compliance issues related to data privacy, labor laws, industry-specific "
                "regulations, and standard business practices."},
...
class FinancialRiskAssessor:
    ...
    def run(self, contract_text):
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a financial analyst specializing in contract risk assessment. "
                "Analyze the contract for financial risks, liability exposure, payment terms issues, "
                "and potential financial implications that could negatively impact a business."},
...
def analyze_contract(contract_text):
    ...
    legelTermsChecker = LegalTermsChecker()
    complianceValidator = ComplianceValidator()
    financialRiskAssessor = FinancialRiskAssessor()

    threads = [
        threading.Thread(target=legelTermsChecker.run, args=(contract_text,)),
        threading.Thread(target=complianceValidator.run, args=(contract_text,)),
        threading.Thread(target=financialRiskAssessor.run, args=(contract_text,)),
    ]
    ...
    return summary_agent.run(contract_text, agent_outputs)
